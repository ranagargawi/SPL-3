...
public class TftpEncoderDecoder<T> implements MessageEncoderDecoder<T> {
    ...
    public byte[] decodeNextByte(byte nextByte) {
        ...
        if (opCodeReceived) {
            ...
            if (len >= 1 && (opCode == 10 || opCode == 6)) {
                return createEasyPackets(opCode, vectorToArray(Vbytes)).getBytes();
            }
            if (len > 1) {
                if (opCode == 4 && len == 3) {
                    byte[] blocks = Arrays.copyOfRange(vectorToArray(Vbytes), 2, 4);
                    short numOfBlocks = bytesToShort(blocks);
                    byte[] ans = packets.new ACKPacket(opCode, numOfBlocks).getBytes();
                    clearFields();
                    return ans;
                }
                if (opCode == 3) {
                    if (len > 5) {
                        ...
                        if (len + 1 == packetSize + 6) {
                            byte[] ans = packets.new DATAPacket(opCode, packetSize,
                                    bytesToShort(Arrays.copyOfRange(vectorToArray(
                                            Vbytes), 4, 6)),
                                    Arrays.copyOfRange(vectorToArray(Vbytes), 6, len + 1)).getBytes();
                            clearFields();
                            return ans;
                        }
                    }
                ...
                if (nextByte == '\0') {
                    if (opCode == 1 || opCode == 2 || (opCode == 5 && len > 3)
                        ...
                        return createDelimiter0Packets(opCode, vectorToArray(Vbytes)).getBytes();
                    }
    ...
    private packets createDelimiter0Packets(short opcode, byte[] decoderBuffer) {
        ...
        switch (opcode) {
            ...
            case 5:
                ...
                s = packets.new ErrorPacket(opcode, errCode, errMsg);
            case 7:
                ...
                s = packets.new LOGRQPakcet(opcode, username3);
                break;
            case 8:
    ...
    public byte[] encode(packets message) {
        byte[] encodedMessage = new byte[((DATAPacket) message).getPacketSize()];
        byte[] tempBytes;
        encodedMessage[0] = shortToBytes(message.getOpCode())[0];
        encodedMessage[1] = shortToBytes(message.getOpCode())[1];
        switch (message.getOpCode()) {
            case 9: // broadcast
                encodedMessage[2] = ((BCASTPacket) message).getDeletedOrAdded();
                ...
                break;
            case 4:// Acknowledge
                encodedMessage[2] = shortToBytes(((ACKPacket) message).getBlockNumber())[0];
                encodedMessage[3] = shortToBytes(((ACKPacket) message).getBlockNumber())[1];
                break;
            case 5:// Error
            ...
            case 3:// Data
                ...
                encodedMessage[4] = shortToBytes(((DATAPacket) message).getNumOfBlock())[0];
                encodedMessage[5] = shortToBytes(((DATAPacket) message).getNumOfBlock())[1];
    ...
    private packets createEasyPackets(short opcode, byte[] myBuffer) {
        ...
        switch (opcode) {
            case 6:
                s = packets.new DIRQPacket(opcode);
                break;
            case 10:
                s = packets.new DISCPacket(opcode);
                break;
